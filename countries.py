...
def country_spread(country_grid, num_countries=3):
    ...
    while changed:
        ...
        for cid in country_ids:
            ...
            while energy > 0:
                ...
                best_land = None
                if land_frontier:
                    best_land = max(land_frontier, key=lambda n: VALUE[n.base - 1])
                ...
                best_tile = max(candidates, key=lambda n: VALUE[n.base - 1])
    ...
    apply_surround_pressure(country_grid, country_ids, num_countries, start_coords)
...
def apply_surround_pressure(country_grid, country_ids, num_passes, starting_tiles):
    """
    A tile may convert if surrounded by neighbours from the same
    different country.
    ...
    for _ in range(num_passes):
        ...
        for tile in country_grid.values():
            ...
            if tile.value not in country_ids:
                continue  # skip non-country tiles

            if tile in protected:
                continue  # skip starting tiles
            ...
            for cid, count in counts.items():
                ...
                break  # Only evaluate strongest pressure source
